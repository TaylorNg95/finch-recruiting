  ...
  return (
    <Box sx={{textAlign: 'center', minHeight: '90vh', display: 'flex', flexDirection: 'column-reverse', justifyContent: 'center', backgroundColor: '#555D50'}}>
      <Box component="form" onSubmit={formik.handleSubmit} sx={{alignItems: 'center'}}>
        <TextField sx={{backgroundColor: '#FFFFFF'}} label="First Name" name='first_name' required variant="filled" value={formik.values.first_name} onChange={formik.handleChange}/>
        <Typography component='p' sx={{color: '#FFFFFF', mb: '1%'}}>{formik.errors.first_name}</Typography>
        <TextField sx={{backgroundColor: '#FFFFFF'}} label="Last Name" name='last_name' required variant="filled" value={formik.values.last_name} onChange={formik.handleChange}/>
